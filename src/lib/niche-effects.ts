...
`;
    ...
    case 'matrix':
      bgHtml = `<canvas id="mg-fx-canvas-matrix" style="position:fixed; top:0; left:0; width:100%; height:100%; z-index:-2; pointer-events:none; background:#020603;"></canvas>`;
      jsEffects = `(function() {
  ...
  function matrix() {
    ...
    ypos.forEach(function(y, ind) {
      ...
      var x = ind * 20; ctx.fillText(text, x, y);
      if (y > 100 + Math.random() * 10000) ypos[ind] = 0;
      else ypos[ind] = y + ${speed === 'fast' ? '24' : speed === 'slow' ? '12' : '18'};
    });
  }
  setInterval(matrix, ${speed === 'fast' ? '30' : speed === 'slow' ? '70' : '50'});
})();`;
      break;

    case 'cyber-grid':
      bgHtml = `
<div id="mg-fx-cybergrid" style="position:fixed; top:0; left:0; width:100%; height:100%; z-index:-2; overflow:hidden; pointer-events:none; background: radial-gradient(circle at 50% 30%, ${brandDark} 0%, #05050f 70%);">
...
`;
    ...
    case 'floating-orbs':
      bgHtml = `
<div id="mg-fx-orbs" style="position:fixed; top:0; left:0; width:100%; height:100%; z-index:-2; overflow:hidden; pointer-events:none; background:#0a0c18;">
...
`;
    ...
    case 'fireflies':
      bgHtml = `
<div id="mg-fx-fireflies" style="position:fixed; top:0; left:0; width:100%; height:100%; z-index:-2; overflow:hidden; pointer-events:none; background:#070810;">
...
`;
    ...
    case 'warp-stars':
      bgHtml = `<canvas id="mg-fx-canvas-warp" style="position:fixed; top:0; left:0; width:100%; height:100%; z-index:-2; pointer-events:none; background:#030008;"></canvas>`;
      jsEffects = `(function() {
  ...
  drawStars();
    ...
    case 'wave-mesh':
      bgHtml = `<canvas id="mg-fx-canvas-waves" style="position:fixed; top:0; left:0; width:100%; height:100%; z-index:-2; pointer-events:none; background:#050714;"></canvas>`;
      jsEffects = `(function() {
  ...
  drawWaves();
    ...
    case 'particles':
    default:
      bgHtml = `<canvas id="mg-fx-canvas-particles" style="position:fixed; top:0; left:0; width:100%; height:100%; z-index:-2; pointer-events:none; background:#080914;"></canvas>`;
      jsEffects = `(function() {
  var canvas = document.getElementById('mg-fx-canvas-particles'); if (!canvas) return;
  var ctx = canvas.getContext('2d');
  var particles = []; var count = window.innerWidth < 768 ? 35 : 55;
  function resize() { canvas.width = window.innerWidth; canvas.height = window.innerHeight; }
  window.addEventListener('resize', resize); resize();
  for (var i = 0; i < count; i++) {
    particles.push({
      ...
      radius: Math.random() * 2.5 + 1, color: '${brand}'
    });
  }
  function draw() {
    ...
    for (var i = 0; i < particles.length; i++) {
      ...
      ctx.beginPath(); ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2); ctx.fillStyle = p.color; ctx.shadowBlur = 8; ctx.shadowColor = p.color; ctx.fill();
