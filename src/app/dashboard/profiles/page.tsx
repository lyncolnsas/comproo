...
export default function ProfilesManage() {
  ...
  if (routerError) {
    return (
      <main className="p-4 md:p-6 space-y-6 animate-fade-in text-slate-800">
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm overflow-hidden">
          <div className="flex items-center gap-2 px-6 py-3 border-b border-slate-200 bg-slate-50">
            <span className="font-mono text-xs font-bold text-slate-700 uppercase">AUDITORIA DE SERVIÇOS DO ROTEADOR</span>
          </div>
          
          <div className="p-6 md:p-8 space-y-6">
            <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
              <div className="w-14 h-14 rounded-xl flex items-center justify-center shrink-0 bg-amber-50 border border-amber-200">
                <span className="text-amber-600 text-2xl">⚠️</span>
              </div>
              <div className="space-y-3 text-center md:text-left flex-1">
                <h3 className="text-base font-bold text-slate-900 uppercase tracking-wider">Hotspot não configurado no Roteador</h3>
                ...
                <div className="bg-slate-100 border border-slate-200 rounded-xl p-3 text-xs text-amber-800 font-mono">
                  {routerError}
                </div>
                <div className="pt-2">
    ...
    <main className="w-full p-4 md:p-6 space-y-6 animate-fade-in text-slate-800">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm overflow-hidden lg:col-span-2">
          <div className="px-6 py-5 border-b border-slate-200 bg-slate-50/70">
            <h2 className="text-sm font-black text-slate-900 uppercase tracking-wider">Novo Perfil de Acesso</h2>
            <p className="text-xs text-slate-500 font-medium mt-0.5">Defina limites, validade e valores para os vouchers</p>
          </div>
          ...
          <form onSubmit={handleCreate} className="p-6 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">IP Pool (Address Pool)</label>
                <select 
                  value={ppool} 
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Limite de Velocidade (Rate Limit)</label>
                <input 
                  type="text" 
                  value={rateLimit} 
                  onChange={(e) => setRateLimit(e.target.value)} 
                  placeholder="Ex: 512k/2M ou 1M/5M" 
                  className="w-full bg-slate-50 border border-slate-300 focus:bg-white focus:border-blue-600 rounded-xl p-2.5 text-xs font-bold text-slate-900 uppercase outline-none shadow-sm placeholder:text-slate-400" 
                />
                <p className="text-[11px] text-slate-500 mt-1 leading-none font-medium">Upload/Download. Vazio para ilimitado.</p>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Usuários Simultâneos (Shared)</label>
                <input 
                  type="number" 
                  min="1" 
                  value={sharedUsers} 
                  ...
                  required
                  className="w-full bg-slate-50 border border-slate-300 focus:bg-white focus:border-blue-600 rounded-xl p-2.5 text-xs font-medium text-slate-900 outline-none shadow-sm" 
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Modo de Expiração</label>
                <select 
                  value={expmode} 
              ...
              {expmode !== '0' ? (
                <>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">Validade (Validity)</label>
                    <input 
                      type="text" 
                      value={validity} 
                      ...
                      className="w-full bg-slate-50 border border-slate-300 focus:bg-white focus:border-blue-600 rounded-xl p-2.5 text-xs font-medium text-slate-900 outline-none shadow-sm placeholder:text-slate-400" 
                    />
                    <p className="text-[11px] text-slate-500 mt-1 leading-none font-medium">Duração após o primeiro login.</p>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">Período de Graça</label>
                    <input 
                      type="text" 
                      value={graceperiod} 
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Vincular ao MAC (Lock User)</label>
                <select 
                  value={lockunlock} 
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Fila Pai (Parent Queue)</label>
                <select 
                  value={parent} 
            ...
            <div className="pt-4 border-t border-slate-200">
              <button 
                type="submit" 
                disabled={creating} 
                className="w-full py-3 px-4 text-xs font-bold rounded-xl bg-blue-600 hover:bg-blue-700 text-white shadow-sm transition-all flex items-center justify-center gap-2 disabled:opacity-50"
              >
        ...
        {/* Readme Panel */}
        <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm overflow-hidden h-fit lg:col-span-1">
          <div className="px-6 py-5 border-b border-slate-200 bg-slate-50/70">
            <h2 className="text-sm font-black text-slate-900 uppercase tracking-wider">Documentação</h2>
            <p className="text-xs text-slate-500 font-medium mt-0.5">Parâmetros de configuração</p>
          </div>
          
          <div className="p-6 space-y-4 text-xs">
            <h3 className="font-extrabold text-sm text-slate-900 uppercase tracking-wider pb-1.5 border-b border-slate-200">
              Guia Rápido
            </h3>
            
            <div className="space-y-4 leading-relaxed text-slate-600 font-medium">
              <div>
                <strong className="text-blue-700 block mb-1 font-bold">1. NOME DO PLANO:</strong>
                <p>Evite espaços em branco. O sistema substitui automaticamente espaços vazios por hífens (`-`).</p>
              </div>
              <div>
                <strong className="text-blue-700 block mb-1 font-bold">2. FORMATO DE VALIDADE:</strong>
              ...
              <div>
                <strong className="text-blue-700 block mb-1 font-bold">3. MODOS DE REGISTRO:</strong>
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-200 bg-slate-50/70 flex justify-between items-center">
          <div>
            <h2 className="text-sm font-black text-slate-900 uppercase tracking-wider">Perfis Cadastrados no MikroTik</h2>
            <p className="text-xs text-slate-500 font-medium mt-0.5">Planos ativos e configurados no roteador</p>
          </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {loading ? (
                  ...
                  <tr key={`${p.id || p.name}-${index}`} className="hover:bg-slate-50/80 transition-colors">
                    <td className="px-6 py-4 font-bold text-blue-600 text-sm">{p.name}</td>
                    <td className="px-6 py-4 text-slate-900 font-medium">{p['shared-users'] || '1'}</td>
                    <td className="px-6 py-4 text-slate-600 font-mono">{p['rate-limit'] || 'Ilimitada'}</td>
                    <td className="px-6 py-4 text-slate-600 font-mono text-right">{p['address-pool'] || 'none'}</td>
                  </tr>
                ))}
